...
1. `WorldSchema` 内的 `character_creation_profile`
   - 定义该世界下允许的基础属性集
   - 定义每个属性的取值范围
   - 定义属性数值区间对应的语义说明
   - 允许附加世界特有属性，但不是强制
   - 定义该世界允许的车卡技能列表
   - 定义技能点总量和 `0/1/2` 三档技能语义
2. `WorldSchema` 内的 `special_status_catalog`
   - 定义该世界可能出现的特殊状态
   - 例如迷惑、污染、祝福、附身、失忆、神话侵蚀
   - 每个状态应包含来源、表现、AI KP 处理约束、恢复条件提示

角色卡阶段不再只依赖轻规则预算，还必须同时服从世界的车卡规范。车卡技能规则不混入世界生成主流程，而是在 world 产物稳定后通过独立脚本生成，便于单独重试、人工复盘和后续 API 化。
...
2. LLM 生成角色草案。
3. 规则检查器验证点数和字段。
4. 世界车卡检查器验证属性范围、属性语义、特殊状态兼容性。
5. 世界观检查器验证设定边界。
6. 模组适配检查器验证动机和剧透风险。
7. 房间顺序审核器在已有已通过角色上下文中检查角色冲突。
8. 输出审核结论与修改建议。

注意：

1. 当前版本中技能不需要由世界或模组预定义。
2. 玩家可自由填写技能，但审核阶段仍要检查技能总量、集中度、是否明显失衡。
3. 单张角色审核与房间上下文审核是两个层次：
   - 单张审核：只看该角色自身是否合理
   - 房间审核：按提交顺序排队，比较与前面已通过角色是否冲突

房间顺序审核至少要覆盖：

1. 是否与已通过角色设定直接矛盾
2. 是否出现重复且互斥的独占身份
3. 是否在公开关系、秘密、核心经历上互相打架
4. 是否导致团队协作结构断裂
